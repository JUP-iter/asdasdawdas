...
Проект уже подготовлен: Vite собирает интерфейс в `dist`, весь Express API экспортируется как единая Vercel Function из `api/index.ts`, а постоянные данные пользователей хранятся в Turso через libSQL.
